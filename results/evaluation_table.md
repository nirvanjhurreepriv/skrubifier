...
**Failure detail (example 06):** Allstate stacking — delta 0.037 vs tolerance 0.020. Root cause: sklearn StackingRegressor uses OOF (cross-validated) predictions for the meta-learner, avoiding leakage. Phase 1's native-decomposition DataOps version applies all branches on the same training set, giving the meta-learner in-fold predictions (mild leakage). No DataOps-native OOF stacking primitive exists in skrub 0.9 for decomposed conversions. Note: wrapping the intact `StackingRegressor` in a single `.skb.apply()` call preserves sklearn's OOF CV semantics and achieves delta=0.000 (see WRITEUP.md §8.1). The gap is specific to native-decomposition conversions — documented, not hidden.
