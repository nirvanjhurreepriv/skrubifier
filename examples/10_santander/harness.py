...
Metric: ROC AUC (binary, rare customer transaction prediction).
Run inside an environment with skrub, scikit-learn, pandas.

NOTE: synthetic train.csv is used (Kaggle Santander Customer Transaction
data unavailable); 200 anonymous numeric features + binary target matches
the real dataset schema.
"""
import json
import os
import runpy
...
learner = ns["learner"]
learner.fit({"santander": train_df})
conv_proba = learner.predict_proba({"santander": test_df})[:, 1]
